...
@onready var base_scale: Vector2 = $Sprite2D.scale
#endregion
...
func _physics_process(delta: float) -> void:
	# 检查是否有冻结
	if hitstop_frames_count > 0:
		hitstop_frames_count -= 1
		if hitstop_frames_count <= 0:
			stop_hitstop()
		return
	...
	# 播放挤压动画，这个动画始终是认为墙在右边的。因此需要旋转sprite
...
func scale_based_on_velocity() -> void:
	if animation_player.is_playing():
		return
	sprite.scale = lerp(base_scale, base_scale * Vector2(1.4, 0.5), velocity.length() / max_speed)
	sprite.rotation = velocity.angle()
	
func spawn_bounce_particles(pos: Vector2, normal: Vector2) -> void:
	var instance = bounce_particlels_scene.instantiate() as Node2D
	get_tree().current_scene.add_child(instance)
	instance.global_position = pos
	instance.rotation = normal.angle()
	
func spawn_bump_particles(pos: Vector2, normal: Vector2) -> void:
	var instance = bump_particlels_scene.instantiate() as Node2D
	get_tree().current_scene.add_child(instance)
	instance.global_position = pos
	instance.rotation = normal.angle()
...
func start_hitstop(frames_amount: int) -> void:
	...
	hitstop_frames_count = frames_amount
	
func stop_hitstop() -> void:
	animation_player.play()
	hitstop_frames_count = 0
